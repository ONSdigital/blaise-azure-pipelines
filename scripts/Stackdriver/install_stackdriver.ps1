...
function Uninstall_Ops_Agent() {
    Write-Output "Attempting to uninstall Ops Agent..."
...
function Uninstall_StackDriver_Logging() {
    if (Check_Service_Exists StackdriverLogging) {
        Write-Output "Stopping StackdriverLogging service"
        Stop-Service -Name StackdriverLogging
    }
}

function Install_StackDriver_Logging($loggingagent, $GCP_BUCKET) {
    Write-Output "Downloading Stackdriver logging agent installer from '$GCP_BUCKET'..."
    gsutil cp gs://$GCP_BUCKET/$loggingagent "C:\dev\data\$($loggingagent)"

    Write-Output "Installing Stackdriver logging agent..."
...
function Install_StackDriver_Monitoring($monitoringagent, $GCP_BUCKET) {
    Write-Output "Downloading Stackdriver monitoring agent installer from '$($GCP_BUCKET)'..."
    gsutil cp gs://$GCP_BUCKET/$monitoringagent "C:\dev\data\$($monitoringagent)"

    Write-Output "Installing Stackdriver monitoring agent..."
...
Write-Output "Target logging agent is: $($loggingagent)"
Write-Output "Target monitoring agent is: $($monitoringagent)"
Write-Output "GCP artifact bucket is: $($GCP_BUCKET)"
